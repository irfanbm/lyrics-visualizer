...
export default function App() {
  ...
  // State: lyrics & editor (tanpa dummy)
  ...
  // Display lines with offset applied
  ...
  // Active index (for line counter display)
  ...
  const handleSeek = (val: number) => { if (audioRef.current) { audioRef.current.currentTime = val; setCurrentTime(val) } }
  
  const handleMark = () => { if (nextIndex >= 0) onMarkAt(nextIndex) }
  
  const onMarkAt = (i: number) => {
  ...
  const handleImportTxt = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        setEditorLines(ed)
        // langsung tampil di player seperti LRC (dummy timing 3s/baris)
        const dummy: LyricLine[] = raw.map((t, i) => {
          const s = i * 3
          const e = s + 2.8
          const ws = t.split(/\s+/).filter(Boolean)
          const per = ws.length ? (e - s) / ws.length : (e - s)
          return { startTime: s, endTime: e, text: t, words: ws.map((w, wi) => ({ text: w, startTime: s + wi * per, endTime: s + (wi + 1) * per })) }
        })
        setLines(dummy)
        setMode('player')
        input.value = ''
  ...
  // Export content using displayLines (with offset!)

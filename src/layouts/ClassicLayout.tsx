...
export function ClassicLayout({ header, previewArea, audioPanel, renderSettingsPanel, textPanel, exportPanel, editorList }: LayoutProps) {
  return (
    <div className="min-h-screen flex flex-col bg-[#0F172A] text-[#F8FAFC]">
      ...
      <div className="flex-1 w-full max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-[380px_minmax(0,1fr)] gap-6 p-4 lg:p-6 items-start">
        <aside className="space-y-4 lg:sticky lg:top-[72px] lg:max-h-[calc(100vh-84px)] lg:overflow-y-auto lg:pr-2 custom-scrollbar order-2 lg:order-1">
          {audioPanel}
          {renderSettingsPanel}
          {textPanel}
          {exportPanel}

...
export function RenderSettingsPanel({ config, onChange }: Props) {
  ...
  const resLabel = (r: string): string => {
    switch (r) { case '1080p': return 'Full HD'; case '720p': return 'HD'; case 'vertical': return '📱 Vertical'; default: return 'Custom' }
  }
  ...
  return (
    <section className="bg-black/40 border border-white/10 rounded-3xl overflow-hidden shrink-0">
      <div className="p-4 border-b border-white/10">
        <h3 className="font-bold text-sm">Render Settings</h3>
        <p className="text-[11px] text-white/50 mt-0.5">Preview = Export (WYSIWYG)</p>
      </div>
      ...
      <div className="p-4 space-y-2 border-b border-white/10">
        ...
        <div className="grid grid-cols-4 gap-1.5">
          <button onClick={() => updateRes('1080p')} className={`px-2 py-2 rounded-lg text-[11px] font-bold transition ${config.resolution === '1080p' ? 'bg-emerald-500 text-black' : 'bg-white/5 hover:bg-white/15 border border-white/10 text-white/60'}`}>1080p</button>
          <button onClick={() => updateRes('720p')} className={`px-2 py-2 rounded-lg text-[11px] font-bold transition ${config.resolution === '720p' ? 'bg-emerald-500 text-black' : 'bg-white/5 hover:bg-white/15 border border-white/10 text-white/60'}`}>720p</button>
          <button onClick={() => updateRes('vertical')} className={`px-2 py-2 rounded-lg text-[11px] font-bold transition ${config.resolution === 'vertical' ? 'bg-emerald-500 text-black' : 'bg-white/5 hover:bg-white/15 border border-white/10 text-white/60'}`}>9:16</button>
          <button onClick={() => updateRes('custom')} className={`px-2 py-2 rounded-lg text-[11px] font-bold transition ${config.resolution === 'custom' ? 'bg-emerald-500 text-black' : 'bg-white/5 hover:bg-white/15 border border-white/10 text-white/60'}`}>Custom</button>
        </div>
        {config.resolution === 'custom' && (
          <div className="grid grid-cols-2 gap-2 pt-2">
            <input type="number" placeholder="Width" value={config.customWidth || 1920} onChange={(e) => onChange({ customWidth: Math.max(16, parseInt(e.target.value) || 1920) })} className="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 text-[11px] outline-none focus:border-emerald-500 text-white" />
            <input type="number" placeholder="Height" value={config.customHeight || 1080} onChange={(e) => onChange({ customHeight: Math.max(16, parseInt(e.target.value) || 1080) })} className="w-full bg-white/5 border border-white/10 rounded-lg px-2 py-1.5 text-[11px] outline-none focus:border-emerald-500 text-white" />
          </div>
        )}
        <p className="text-[10px] text-white/40 pt-1">Current: {getDimensions(config)}</p>
      </div>

      {/* Lyric Position & Font */}
      <div className="p-4 space-y-3 border-b border-white/10">
        <label className="text-[11px] text-white/60 block">Lyric Position</label>
        <div className="grid grid-cols-3 gap-1.5">
          {(['top', 'center', 'bottom'] as const).map(pos => (
            <button key={pos} onClick={() => onChange({ lyricPosition: pos })} className={`px-2 py-2 rounded-lg text-[11px] font-bold transition capitalize ${config.lyricPosition === pos ? 'bg-emerald-500 text-black' : 'bg-white/5 hover:bg-white/15 border border-white/10 text-white/60'}`}>{pos}</button>
          ))}
        </div>
      </div>

      <div className="p-4 space-y-3 border-b border-white/10">
        <label className="text-[11px] text-white/60 block">Google Font Family</label>
        <select value={config.fontFamily} onChange={(e) => onChange({ fontFamily: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-lg px-2.5 py-2 text-[11px] outline-none focus:border-emerald-500 text-white appearance-none">
      ...
      <div className="p-4 space-y-2">
        ...
        {[
          ...
          <label key={k} className="flex items-center gap-2.5 text-[11px] cursor-pointer select-none">

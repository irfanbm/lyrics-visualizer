  ...
  return (
    <section className="glass rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.35)] overflow-hidden shrink-0">
      <div className="p-4 border-b border-white/10">
        <h3 className="font-bold text-sm font-display">Import Text</h3>
      ...
      <div className="p-2.5 space-y-2">
        <textarea
          ...
          rows={6}
          aria-label="Paste lyrics text"
          className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs outline-none focus:border-[var(--color-accent)] text-white resize-none custom-scrollbar min-h-[110px]"
        />
        <button
          onClick={onPasteToEditor}
          className="w-full min-h-[36px] py-2.5 bg-[var(--color-accent)] hover:bg-[#16A34A] neon-glow text-black font-bold rounded-xl text-xs transition"
          aria-label="Send pasted text to editor"
        >
          Send to Editor
        </button>
        <div className="text-[10px] text-white/40 leading-relaxed">
          Tip: Untuk word-by-word karaoke, import file LRC enhanced:

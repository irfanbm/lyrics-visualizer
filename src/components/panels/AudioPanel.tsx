        ...
        {mode === 'editor' && (
          <div className="pt-2 border-t border-white/10 space-y-2">
            <div className="flex items-center justify-between text-[11px] font-bold">
              <span className="text-amber-200">Marking Mode</span>
              <span className="text-amber-200/70">{markedCount}/{totalLines} marked</span>
            </div>
            <button
              onClick={onStartFromZero}
              className="w-full py-2.5 rounded-xl bg-amber-500 hover:bg-amber-400 text-black font-bold text-xs transition"
            >
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 py-3 rounded-xl text-xs font-bold transition ${nextIndex >= 0 ? 'bg-emerald-500 text-black' : 'bg-white/5 text-white/40 cursor-not-allowed'}`}
              >
                ● Mark #{nextIndex >= 0 ? nextIndex + 1 : 'done'} (Space)
              </button>
